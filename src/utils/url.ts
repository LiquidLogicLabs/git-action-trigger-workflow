 ...
 */
export function safeSegment(value: string | number, label: string): string {
  const s = String(value);
  if (s === '.' || s === '..') {
    throw new Error(
      `Refusing to use ${JSON.stringify(s)} as a ${label}: it would redirect the request to a different endpoint.`
    );
  }
